...
# run_specex.sh
# Helper script to run origin/codex/spec-ex-correction-consistency locally.
# Commands:
...
STATE_DIR="${ROOT_DIR}/tmp/run_specex"
LOG_DIR="${STATE_DIR}/logs"
PID_DIR="${STATE_DIR}/pids"

SERVICES=(api ws-gateway proxy voip-client ngrok)
...
usage_platform_note() {
  cat <<'EOF_USAGE_PLATFORM'

Platform notes:
  - Supported: macOS, Linux
  - macOS needs Xcode Command Line Tools for native modules (better-sqlite3):
...
Usage:
  ...
  ./docs/run_specex.sh logs [api|ws-gateway|proxy|voip-client|ngrok]
  ./docs/run_specex.sh help
...
Optional environment variables:
  ...
  SKIP_VOIP                 (default: 0; set 1 to skip voip-client start)
  INSTALL_DEPS              (default: 1; set 0 to skip npm ci)
  RUN_NGROK                 (default: 0; set 1 to start ngrok)
  ...
  AUTO_NGROK_STREAM         (default: 1; auto inject STREAM_URL/STATUS when RUN_NGROK=1)
  AUTO_WRITE_VOIP_ENV       (default: 1; write STREAM_URL/STATUS to voip-client/.env)
  AUTO_CREATE_VOIP_ENV      (default: 1; create .env from .env.example if missing)
  VOIP_ENV_FILE             (default: <repo>/voip-client/.env)
  SKIP_PLATFORM_CHECK       (default: 0; set 1 to skip platform/toolchain checks)
  TAIL_LINES                (default: 80)
  FOLLOW                    (default: 0; set 1 to follow logs)
  AUTO_OPEN_BROWSER         (default: 1; open http://localhost:<VOIP_PORT> after startup)

Examples:
  OPENAI_API_KEY=... REALTIME_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe ./docs/run_specex.sh up
  RUN_NGROK=1 OPENAI_API_KEY=... REALTIME_TRANSCRIPTION_MODEL=gpt-4o-mini-transcribe ./docs/run_specex.sh up
...
assert_spec_ex_layout() {
  [[ -f "${ROOT_DIR}/ws-gateway/catalog.json" ]] || die "ws-gateway/catalog.json not found. Use spec-ex branch/worktree."
  [[ -f "${ROOT_DIR}/ws-gateway/inventory.json" ]] || die "ws-gateway/inventory.json not found. Use spec-ex branch/worktree."
}
...
extract_ngrok_url_from_json() {
  # Use -c so stdin stays available for piped JSON.
  python3 -c '
import json
import sys
...
prepare_api() {
  ...
  (
    cd "${ROOT_DIR}/api"
    DB_PATH="$DB_PATH" npm run init-db
    DB_PATH="$DB_PATH" ADMIN_EMAIL="$ADMIN_EMAIL" ADMIN_PASSWORD="$ADMIN_PASSWORD" ADMIN_NAME="$ADMIN_NAME" npm run seed-admin
  )
}
