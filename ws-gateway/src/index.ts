...
const buildStrictRendererInstructions = (text: string) =>
  [
    "あなたは状態機械の音声レンダラーです。",
    "次の文章を一字一句そのまま日本語で読み上げてください。",
    "言い換え・補足・要約・追加質問は禁止です。",
    `文章: ${text}`,
  ].join("\n");
...
wss.on("connection", (ws: WebSocket) => {
  ...
  const sendToRealtime = (payload: object) => {
    if (!realtime || !realtime.isOpen()) return;
  ...
  const createResponse = (instructions?: string) => {
    ...
    const responseInstruction = buildStrictRendererInstructions(spokenText);
  ...
  realtime = createRealtimeConnection({
    transport: config.realtimeTransport,
    apiKey: config.openaiApiKey,
    model: config.realtimeModel,
    realtimeUrl: config.realtimeUrl,
    realtimeApiBaseUrl: config.realtimeApiBaseUrl,
    includeBetaHeader: config.realtimeBetaHeader,
  });
  realtime.onOpen(() => {
  ...
  realtime.onMessage((payload) => {
    if (payload.type === rawBinaryAudioEventType) {
      const audio = payload.audio as string | undefined;
      if (audio) {
        sendToTwilio(audio);
        console.log(`${logPrefix(wsId)} realtime binary audio`, Buffer.from(audio, "base64").length);
      }
      return;
    }
    ...
    const userTranscript = extractUserTranscript(payload as RealtimeEvent);
    if (userTranscript) {
      const now = Date.now();
      if (
        pendingTranscript &&
    ...
    if (payload.type === "session.created" && !sessionCreatedLogged) {
      ...
      validateSession(payload as RealtimeEvent);
    }
  ...
  realtime.onClose((code, reason) => {
    ...
    realtimeSessionReady = false;
    console.log(`${logPrefix(wsId)} realtime closed`, { code, reason });
  });
...
server.listen(config.port, () => {
  ...
  console.log(`Realtime transport: ${config.realtimeTransport}`);
  if (config.realtimeTransport === "raw_ws") {
    console.log(`Realtime URL: ${config.realtimeUrl}`);
  } else {
    console.log(`Realtime API Base URL: ${config.realtimeApiBaseUrl}`);
  }
